...
- **gRPC microservices** — Set `server.mode: micro` and add `grpc_server` / `grpc_clients` for inter-service communication. Pair the `type: grpc` entry with `svc.RegisterGrpcService()` to auto-wire proto servers. In monolith mode (default), gRPC is disabled. See `examples/600-grpc/` for 8 connected services with 153 integration tests.
- **Custom metrics** — Set `prometheus.enabled: true` to activate `infra/metric` counters and expose them on `/metrics`
