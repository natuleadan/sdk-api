...
func TestMain(m *testing.M) {
	...
	if !docker {
		...
		out, err := exec.Command("go", "build", "-buildvcs=false", "-tags", "golibsql", "-o", "/tmp/url-svc", ".").CombinedOutput()
		if err != nil {
...
func setup(tb testing.TB) {
	...
	// Remote variants require connection credentials outside Docker.
	if os.Getenv("TURSO_DATABASE_URL") == "" && os.Getenv("BUNNY_DATABASE_URL") == "" {
		tb.Skip("TURSO_DATABASE_URL/BUNNY_DATABASE_URL not set (run via docker-compose with credentials)")
	}
	setupOnce.Do(func() {
		tbLog(tb, "starting service...")
		svcCmd = exec.Command("/tmp/url-svc")
		// Remote variants need the TURSO_DATABASE_URL/TURSO_AUTH_TOKEN env
		// (or BUNNY_DATABASE_URL/BUNNY_DATABASE_AUTH_TOKEN) to connect.
