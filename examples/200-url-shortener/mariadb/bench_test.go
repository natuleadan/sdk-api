...
func waitMariaDB(tb testing.TB) {
	for i := range 20 {
		if run("docker", "compose", "exec", "-T", "mariadb", "mysqladmin", "ping", "-u", "dev", "-pdevpass", "--silent") == nil {
			return
		}
		tb.Logf("waiting for mariadb... (%d/20)", i+1)
		time.Sleep(500 * time.Millisecond)
	}
	tb.Fatal("mariadb timeout")
}
