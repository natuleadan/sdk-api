...
func waitMongo(tb testing.TB) {
	for i := range 20 {
		if run("docker", "compose", "exec", "-T", "mongo", "mongosh", "--quiet", "--eval", "db.runCommand('ping')") == nil {
			return
		}
		tb.Logf("waiting for mongo... (%d/20)", i+1)
		time.Sleep(500 * time.Millisecond)
	}
	tb.Fatal("mongo timeout")
}
...
func TestURL_CreateAndExpand(t *testing.T) {
	...
	body := `{"targetUrl":"https://example.com/test-create","shortCode":"test-short"}`
	resp, err := http.Post(apiLinks, "application/json", strings.NewReader(body))
	if err != nil {
		t.Fatalf("POST: %v", err)
	}
...
func TestURL_CRUDLifecycle(t *testing.T) {
	...
	if !ok || id == "" {
		t.Fatal("no _id returned")
	}
	t.Logf("created _id=%s", id)

	getResp, err := http.Get(fmt.Sprintf("%s/%s", apiLinks, id))
	if err != nil {
		t.Fatalf("get: %v", err)
	}
	...
	req, _ := http.NewRequest("DELETE", fmt.Sprintf("%s/%s", apiLinks, id), nil)
