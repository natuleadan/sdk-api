...
for i in $(seq 1 15); do
	curl -s --max-time 3 http://localhost:23204/health >/dev/null 2>&1 && break
	sleep 1
done
...
if [ "$RPS_BENCH" = "1" ]; then
	...
	bench_one() {
		local label=$1 lua=$2
		echo "--- $label warmup ---"
		wrk -t10 -c1000 -d30s -s "/app/$lua" --latency "http://localhost:23204" 2>&1 | awk '/Requests\/sec/ {print "  warmup:", $2}'
		sleep 2
		echo "--- $label measure ---"
		wrk -t10 -c1000 -d30s -s "/app/$lua" --latency "http://localhost:23204" 2>&1 | awk '/Requests\/sec/ {print "  measure:", $2}'
		sleep 1
	}
