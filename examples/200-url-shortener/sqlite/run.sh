...
if [ "$RPS" = "true" ]; then
	...
	bench_one() {
		local label=$1 lua=$2
		echo "--- $label warmup ---"
		wrk -t10 -c1000 -d3s -s "/app/$lua" --latency "http://localhost:23208" 2>&1 > /dev/null
		echo "--- $label measure ---"
		wrk -t10 -c1000 -d5s -s "/app/$lua" --latency "http://localhost:23208" 2>&1 | awk '/Requests\/sec/ {print "  measure:", $2}'
	}
