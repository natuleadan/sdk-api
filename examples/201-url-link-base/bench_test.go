...
func TestMain(m *testing.M) {
	...
	if !docker {
		...
		out, err := exec.Command("go", "build", "-buildvcs=false", "-o", "/tmp/url-link-base-svc", ".").CombinedOutput()
		if err != nil {
...
func setup(tb testing.TB) {
	...
	setupOnce.Do(func() {
		...
		run("docker", "compose", "up", "-d", "postgres", "redis", "pgdog")
		waitPostgres(tb)

		tbLog(tb, "starting service...")
		svcCmd = exec.Command("/tmp/url-link-base-svc")
		svcCmd.Env = append(os.Environ(),
			"DATABASE_URL=postgres://dev:devpass@localhost:15431/postgres?sslmode=disable",
			"PORT="+httpPort,
		)
...
func seedHotKeys(tb testing.TB, n int) {
	for i := range n {
		body := fmt.Sprintf(`{"targetUrl":"https://hot-%d.example.com"}`, i)
...
func TestURLLink_CreateAndExpand(t *testing.T) {
	setup(t)
	...
	resp2, err := http.Get(apiExpand + "/" + code)
	if err != nil {
		t.Fatalf("expand: %v", err)
	}
	defer resp2.Body.Close()
	if resp2.StatusCode != 200 {
		t.Errorf("expand: expected 200, got %d", resp2.StatusCode)
	}
	var result map[string]any
	json.NewDecoder(resp2.Body).Decode(&result)
	if url, _ := result["url"].(string); url != "https://example.com/test-create" {
		t.Errorf("expand: expected url, got %v", result)
	}
}

func TestURLLink_CRUDLifecycle(t *testing.T) {
	setup(t)
