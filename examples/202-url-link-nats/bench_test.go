...
func setup(tb testing.TB) {
	...
	setupOnce.Do(func() {
		...
		run("docker", "compose", "up", "-d", "postgres", "nats", "pgdog")
		waitPostgres(tb)

		tbLog(tb, "starting service...")
		svcCmd = exec.Command("/tmp/url-link-nats-svc")
		svcCmd.Env = append(os.Environ(),
			"DATABASE_URL=postgres://dev:devpass@localhost:15430/postgres?sslmode=disable",
...
func TestURLLink_CreateAndExpand(t *testing.T) {
	...
	body := `{"targetUrl":"https://example.com/test-nats"}`
	resp, err := http.Post(apiLinks, "application/json", strings.NewReader(body))
	if err != nil {
		t.Fatalf("POST: %v", err)
	}
	...
	if url, _ := result["url"].(string); url != "https://example.com/test-nats" {
...
func TestURLLink_CRUDLifecycle(t *testing.T) {
	...
	body := `{"targetUrl":"https://lifecycle-nats.example.com"}`
	resp, err := http.Post(apiLinks, "application/json", strings.NewReader(body))
	if err != nil {
