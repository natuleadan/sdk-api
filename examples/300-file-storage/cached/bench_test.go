...
func TestMain(m *testing.M) {
	...
	if !docker {
		...
		out, err := exec.Command("go", "build", "-buildvcs=false", "-o", "/tmp/file-cached-svc", ".").CombinedOutput()
		if err != nil {
...
func setup(tb testing.TB) {
	...
	setupOnce.Do(func() {
		...
		svcCmd.Env = append(os.Environ(), "PORT=10124")
...
func TestFile_UploadAndDownload(t *testing.T) {
	...
	payload := "hello-file-cached-" + code
...
func TestFile_CacheHitRAM(t *testing.T) {
	setup(t)
	...
	if r1.StatusCode != 200 {
		t.Fatalf("first download: %d", r1.StatusCode)
	}
	if string(body1) != payload {
		t.Fatalf("first download content: got %q, want %q", string(body1), payload)
	}
	...
	if string(body2) != payload {
		t.Fatalf("second download content: got %q, want %q", string(body2), payload)
	}
}
