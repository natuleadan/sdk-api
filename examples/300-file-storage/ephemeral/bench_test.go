...
func TestMain(m *testing.M) {
	...
	if !docker {
		...
		out, err := exec.Command("go", "build", "-buildvcs=false", "-o", "/tmp/file-ephemeral-svc", ".").CombinedOutput()
		if err != nil {
...
func TestFile_UploadAndDownload(t *testing.T) {
	...
	payload := "hello-file-ephemeral-" + code
...
func TestFile_ListFiles(t *testing.T) {
	setup(t)
	for i := range 3 {
...
func TestFile_DeleteFile(t *testing.T) {
	...
	if resp.StatusCode != 200 {
		body, _ := io.ReadAll(resp.Body)
		t.Fatalf("delete status %d: %s", resp.StatusCode, string(body))
	}

	resp2, _ := http.Get(apiDownload + "/" + code + ".dat")
	defer resp2.Body.Close()
