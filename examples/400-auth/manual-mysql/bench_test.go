...
// rawPool exposes the configured database through database/sql so the harness
// can run portable SQL regardless of the service driver.
type rawPool struct {
	db *sql.DB
	d  db.SQLDialect
}

func openRaw(url string) (*rawPool, error) {
	driver := os.Getenv("DB_DRIVER")
	if driver == "" {
		driver = "postgres"
	}
	sqlDB, err := db.OpenStdlib(driver, url)
	if err != nil {
		return nil, err
	}
	return &rawPool{db: sqlDB, d: db.DialectForDriver(driver)}, nil
}

func (p *rawPool) Exec(ctx context.Context, query string, args ...any) (sql.Result, error) {
	query, args = db.Rewrite(p.d, query, args...)
	return p.db.ExecContext(ctx, query, args...)
}

func (p *rawPool) Query(ctx context.Context, query string, args ...any) (*sql.Rows, error) {
	query, args = db.Rewrite(p.d, query, args...)
	return p.db.QueryContext(ctx, query, args...)
}

func (p *rawPool) QueryRow(ctx context.Context, query string, args ...any) *sql.Row {
	query, args = db.Rewrite(p.d, query, args...)
	return p.db.QueryRowContext(ctx, query, args...)
}

func (p *rawPool) Close() error { return p.db.Close() }
...
func TestMain(m *testing.M) {
	...
	if p, err := openRaw(poolURL); err == nil {
...
func insertAccessCode(t *testing.T, userID, code, purpose, deliveredTo string, expiresIn int) {
	...
	_, err = p.Exec(ctx,
		`INSERT INTO auth_codes (user_id, code, purpose, delivered_to, delivery_method, expires_at)
		 VALUES ($1, $2, $3, $4, 'test', $5) ON CONFLICT DO NOTHING`,
		userID, code, purpose, deliveredTo, time.Now().Add(time.Duration(expiresIn)*time.Second))
	if err != nil {
