...
func handleOAuthClientsList(svcCtx *svc.ServiceContext) func(c *runtime.RestCtx) error {
	return func(c *runtime.RestCtx) error {
		...
		for rows.Next() {
			...
			var grantTypes, responseTypes oauthstore.StringSlice
			var isPublic bool
			rows.Scan(&id, &grantTypes, &responseTypes, &scopes, &isPublic)
			clients = append(clients, runtime.Map{
				"id": id, "grant_types": []string(grantTypes),
				"response_types": []string(responseTypes), "scopes": scopes, "is_public": isPublic,
			})
		}
		if clients == nil {
			clients = []runtime.Map{}
...
func handleOAuthClientsCreate(svcCtx *svc.ServiceContext) func(c *runtime.RestCtx) error {
	return func(c *runtime.RestCtx) error {
		...
		_, err := pool.Exec(c.Context(),
			...
			body.ID, hashed,
			oauthstore.StringSlice(body.RedirectURIs),
			oauthstore.StringSlice(body.GrantTypes),
			oauthstore.StringSlice(body.ResponseTypes),
			body.Scopes, oauthstore.StringSlice{}, body.IsPublic)
		if err != nil {
			return c.Status(500).JSON(runtime.Map{"code": 500, "message": err.Error()})
		}
