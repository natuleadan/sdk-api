...
type ServiceContext struct {
	...
	// RolePermissions maps each role to the permissions it natively holds
	// (wildcards allowed). Delegated grants extend this at runtime.
...
// SetRolePermissions records the role → permissions table used for delegated
// grants (CanDelegate) and effective-permission resolution.
func (s *ServiceContext) SetRolePermissions(rp map[string][]string) {
	s.RolePermissions = rp
}

// InitOAuth wires the OAuth2 provider over the driver-agnostic store.
func (s *ServiceContext) InitOAuth(store *oauthstore.Store) {
	s.OAuth = NewOAuthProvider(store, s.JWTSecret)
}
...
func (s *ServiceContext) HashPassword(password string) (string, error) {
	return sdkauth.HashPassword(password)
}

func (s *ServiceContext) VerifyPassword(hash, password string) bool {
	return sdkauth.VerifyPassword(hash, password)
}
...
var roleHierarchy = sdkauth.RoleHierarchy{
	"viewer": {},
	"editor": {"viewer"},
	"admin":  {"editor", "viewer"},
}
...
func (s *ProductStore) GetAuditLog(c *runtime.RestCtx) error {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&e.ID, &e.ProductID, &e.Action, &e.ChangedBy, &oldVal, &newVal, &createdAtRaw); err != nil {
			break
		}
		if ts, terr := db.ParseTime(createdAtRaw); terr == nil {
