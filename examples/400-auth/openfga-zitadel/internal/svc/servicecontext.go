...
// SetFGA wires the OpenFGA client used for role assignment (authorization
// checks are enforced by the SDK middleware from entry.roles).
func (s *ServiceContext) SetFGA(client *openfga.Client) {
	s.FGA = client
}
...
func (s *ProductStore) hasRole(a *middleware.AuthContext, role string) bool {
	// Authorization is 100% OpenFGA: the middleware already enforced
	// entry.roles against the FGA store, so the in-handler check is a no-op.
	_ = a
	_ = role
	return true
}
