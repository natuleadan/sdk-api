...
// handleSetUserRole writes the role assignment to Ory Keto
// (user → assignee → roles:<role>). Role names are free-form; authorization is
// Keto-backed.
func handleSetUserRole(svcCtx *svc.ServiceContext) func(c *runtime.RestCtx) error {
	return func(c *runtime.RestCtx) error {
		...
		if err := svcCtx.Ory.WriteKetoTuple(context.Background(), "roles", body.Role, "assignee", "user:"+id); err != nil {
